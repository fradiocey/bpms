...
//get file name
sql.execute( { 
 query: "SELECT id, ProjectCode, ProjectName, ConcreteCover, SteelDiameter FROM BplProject where id = 5" 
...
//str = str.replace(/\s/g, "X"); 1/12/2016
str = str.replace(/ /g,'');
var MainBar = str.split("T");
var MainBar0 = MainBar[0];
var MainBar1 = MainBar[1];
...
if (isNaN(parseFloat(MainSteelBar))){
sheet1.set(22, i, "-");//newreq 
}
else{
...
if (isNaN(parseFloat(finalMBArr[BDSB]))){
...
if (isNaN(parseFloat(FinalTotalLofHL))){
...
if (isNaN(parseFloat(finalArr[BDM]))){
 sheet1.set(25+BDBL+BDM , i, "-");//newreq 
}
else{
...
if (TotalBDRCArr1 == ""){
 sheet1.set(22+RCAdditional, totaldatatot-1, 0.0);//new
}else{

if (isNaN(parseFloat(TotalBDRCArr))){
sheet1.set(22+RCAdditional, totaldatatot-1, "-");//new 
}
else{
...
if (keyObj[keyi] = uniqueRCArr[titlei]){
 ...
 if (isNaN(parseFloat(valueRC))){
  sheet1.set(23+titlei, totaldatatot-1,parseFloat(0).toFixed(1));//new
}
else{
...
if (calHLtot != 0){
for (var ttHL = 0; ttHL < calHLtot; ttHL++ ){
// loop to get T
...
if (isNaN(parseFloat(TotalBDHLArr))){
sheet1.set(24+RCAdditional+HLAdditional, totaldatatot-1, parseFloat(0).toFixed(1));//new
}
else{
...
if (isNaN(parseFloat(valueHL))){
sheet1.set(25+titleHLi+RCAdditional, totaldatatot-1, parseFloat(0).toFixed(1) );//new
}
else{
